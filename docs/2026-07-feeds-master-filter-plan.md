...
> ステータス: 計画確定(2026-07-20)。`feature/feeds_master_filter` にコミット1(docs)・コミット2(feat)・コミット3(refactor)を実装済み、全機械ゲート緑(2026-07-20)。push・PR 作成・マージは人間の担当で未実施。
>
...
- **PR は 1 本、コミットは 2 つ**(コミット1 = docs、コミット2 = feat)。各コミットは機械ゲート通過状態であること
  - **実績: コミット3(refactor)が加わって3コミットになった。** コミット2 の完了後、audit-plan-step の監査が「early return の挙動変更を守る自動テストが1件もない(index.ts のテストは元々存在せず、verify のフィクスチャは登録済み品種を含むため0件経路を踏まない)」と指摘。ユーザー判断で本 PR の範囲内として、dataset 組み立てを純関数に抽出しテストを足した(下記「コミット3」参照)
...
### コミット3: refactor(監査指摘への対応・計画策定時には想定していなかった)

コミット2 の監査指摘(自動テストのない挙動変更)を恒久ゲート化するために追加した。

- `calculateTotalByNameAndMonth` の dataset 組み立てループを、新規モジュール `packages/ringo-db/src/aggregation.ts` の純関数 `buildMonthlyDatasetsPure(colorMap, rows)` に抽出する。0件時に空配列を返す判定も関数内に移す(index.ts の early return は消える)
- `varietyData.ts` に colorMap を束縛した `buildMonthlyDatasets` ラッパーを足す(`filterRegisteredRows` と同じパターン)
- `aggregation.test.ts` を新設し、0件・単一品種・複数品種の境界(末尾グループの取りこぼし)・data 配列の非共有・色フォールバックを検証する
- 合格条件: `bun test` が緑、`./verify/verify.sh` が exit 0 で **baseline 無変更**(挙動が変わっていないことの証明)。加えて、抽出した0件判定を一時的に外すとテストが落ちることを確認する(ゲートが実際に効いている証明)
- **`varietyData.ts` の `findColorName` はこの抽出で未使用の export になったため、このコミットで削除した。** ラッパー(`findColorNamePure` の束縛)の除去のみで、色解決ロジック本体(`genealogy.ts` の `findColorNamePure` とその `'red'` フォールバック)には手を付けていない。`findColorNameOrUndefined` は `calculateTotalByName` が現役で使うため残す
...
- **色フォールバック(`findColorNameOrUndefined` / `'red'`)の整理は先送り**。本 PR ではフィルタ追加に集中し、整理はフィルタ稼働後の独立 refactor とする。ただしコミット3 の抽出で完全に未参照になった `varietyData.ts` の `findColorName` だけは、デッドコードを残さないためコミット3 で削除した(色解決ロジック本体は温存)
