...
コミットの手順とメッセージはグローバルの `thinkami-git-commit` スキルに従う。このリポジトリ固有の上書きは次のとおり
（対象は人間・AI が作成するコミット。bot（Dependabot 等）が作るコミットは対象外で `chore(deps): ...` 形式に設定済み）:

- メッセージは日本語（type / scope は英語のまま。要約は50字目安・句点なし）
...
- 本文は `なぜ:` で始まる行を**無条件で**持つ（「自明なら省略」はしない。audit-plan-step が機械検査するスロットのため）。理由は発明せず、上流の成果物（計画ステップ・完了報告・依頼文）から転記する
- push・PR 作成は人間の担当
