...
> ステータス: 計画確定(2026-07-15)。登録42件を実装済み(2026-07-17)。監査で出典7件・系譜2件を訂正済み(2026-07-20、機械ゲート緑・コミット未承認)。見送り(基本品種)13件 / 先送り38件 / 出典なし19件は現状維持。
>
> **この計画の「確定した調査結果テーブル」は当時のスナップショットで、最新値ではない。** 系譜・出典の現行値は `packages/ringo-db/data/varieties.md` が正。差分の一覧と理由は「実装後の訂正」を参照。
>
...
- 本計画は grill-with-docs の設計対話で全方針を確定し、全既存品種の調査結果を下記「確定した調査結果テーブル」に転記済み。**実装セッションは Web 調査をやり直さず、テーブルの通りに varieties.md を編集するだけ**(データ変換は機械的作業。判断はこの計画段階で完了している)
  - **ただしこの指示は初回実装(2026-07-17)で役目を終えている。** 以後は varieties.md が現行値であり、テーブルはその時点の調査記録。**登録済みの行をテーブルに合わせて書き戻す作業は行わない**(「実装後の訂正」参照)
...
> **⚠️ この表は 2026-07-15 時点の調査結果スナップショットであり、最新の値ではない。系譜・出典の source of truth は `packages/ringo-db/data/varieties.md`。**
> 実装後の監査(2026-07-20)で出典基準違反が見つかり、下記「実装後の訂正」の8件は varieties.md 側だけを直して**この表は当時の記録として意図的に残している**。
> **この表と varieties.md が食い違っていても、表に合わせて varieties.md を書き換えてはならない。** 差分は訂正の履歴であって不整合ではない。
> varieties.md をこの表の値に戻す変更は、下記の訂正理由を無効化する新たな根拠がある場合にのみ、ユーザーの承認を得て行う。

> このセクションは grill/write-plan 段階の Web 調査結果。初回実装時、実装セッションはこの表の通りに varieties.md を編集した(再調査不要)。分類の意味:
...
### 実装後の訂正（2026-07-20 監査・上の表より優先する）

audit-plan-step による独立監査で、上の表の出典が「出典の採否基準」に反する / 記載内容を裏付けないケースが見つかった。
**varieties.md 側のみを訂正済みで、上の表は当時の調査結果として保存している。** 現行値は必ず varieties.md を見ること。

| 品種 | 上の表の値 | varieties.md の現行値 | 訂正理由 |
|---|---|---|---|
| 金星 | 花粉親 `kokkou` / makotononikki | 花粉親 `unknown` / 青森県のりんご 改訂版 p77 | 出典が花粉親を「デリシャス系品種のりんご」とし、国光を裏付けていなかった |
| はるか | 花粉親 `unknown` | 花粉親 `starking` | 出典(りんご大学)が花粉親「スターキングデリシャス」と明記しており、`starking` は同 PR で登録済み |
| 未来ふじ | りんご大学 ふじ(枝変わり) | [福島天香園](https://fukuten.com/product/item.html?id=96) | 旧出典のページに「未来ふじ」が一度も出現せず、当該品種を裏付けていなかった |
| ジャンボ王林 | Wikipedia | 青森県のりんご 改訂版 p129 | 採否基準が「Wikipedia 単独は根拠にしない」と明示的に禁じていた |
| おいらせ | makotononikki | 青森県のりんご 改訂版 p49 | 個人ブログ単独で、採否基準4の「複数の独立情報源で一致」要件を満たさない |
| 秋陽 | makotononikki | 青森県のりんご 改訂版 p130 | 同上 |
| 紅将軍 | foodslink | 青森県のりんご 改訂版 p227 | 旧出典は「やたかの枝変わり」までしか裏付けず、`やたか←ふじ` の一段が無出典だった |

**ロマンシリーズ2件の出典追記（2026-07-20・計画の42件の対象外）**: `ゴールドロマン`・`奥州ロマン` は本計画以前から系譜が入っていた既存行だが、監査中に出典の不備が見つかったためユーザー指示で追記した。**系譜の値は変更していない（出典セルのみ追加）。**

- `ゴールドロマン`: 既存出典の[品種登録DB 登録番号20601](https://www.hinshu2.maff.go.jp/vips/cmm/apCMM112.aspx?TOUROKU_NO=20601&LANGUAGE=Japanese)は**交配親を記載していない**ため、系譜の根拠になっていなかった。[紅果園 ロマンシリーズ(PDF)](https://kokaen.jp/pdf/romanseries.pdf)を ` / ` 区切りで追記。**2出典で1つの意味をなす**構成（登録DB = ゴールドロマン ≡ 高野3号 の同一性、PDF = 交配親）
- `奥州ロマン`: 出典が空だったので同PDFを記入
- PDF の記載は両品種とも「交配親: シナノゴールド × つがる」で、記録済みの `種子親=shinano_gold` / `花粉親=tsugaru` と一致（`A × B` は A=種子親、B=花粉親）。同PDFの `紅ロマン`「シナノレッド × さんさ」は先送り表の値とも一致する
- 同一性の裏付け: PDF の品種登録日 2011.3.15 が登録番号20601 の登録年月日 2011/03/15 と一致

**凛夏（りんか）の再調査（2026-07-20）**: 上の表は「ニホンナシの疑い・出典なし」としていたが、[品種登録DB 登録番号14918](https://www.hinshu2.maff.go.jp/vips/cmm/apCMM112.aspx?TOUROKU_NO=14918&LANGUAGE=Japanese) で次が確定した。**この行を「梨だから」という理由で削除してはならない。**

- 農林水産植物の種類は **`Malus Mill.`（和名: リンゴ属）**、作物区分は果樹 → **リンゴで確定**。梨説は誤り
- 特性の概要の原文: 「この品種は、**「つがる」に「さんさ」を交配して**育成されたものであり」→ **種子親(母) = つがる / 花粉親(父) = さんさ**
- 育成者 工藤凛三、品種登録者 株式会社天香園、育成地 岩手県二戸市、登録 2007/03/02
- **ただし現時点では登録できない。** `さんさ` は品種マスタに行はあるが `name` 未設定で、参照整合 lint が通らない。`さんさ`(= ガラ × あかね) の登録には祖先 `あかね`・`ガラ` の追加が必要 → **凛夏の分類は「出典なし」ではなく「先送り」**。祖先追加ステップで `さんさ` と同時に登録する
- 出典セルのみ先行して記入済み（系譜3セルは空のまま。lint の「3セルとも空 or 3セルとも埋める」規則を満たす）

- `早生ふじ` は上の表のまま維持する（ユーザー判断）。出典ページは「早生系ふじ」を総称と説明するが、**食べた実物のラベルが「早生ふじ」としか読めない場合に記録できる先が必要**なため、品種行として残す
- この訂正により `/api/genealogies` の `はるか`・`金星` の花粉親が変わり、`verify/baseline/genealogies.json` は訂正後の値で再生成済み
...
| 凛夏 | ~~**りんごではなくニホンナシの品種の疑いが濃厚**（農研機構が梨として育成）。行の削除は今回スコープ外のため要別途相談~~ → **2026-07-20 に否定された。リンゴで確定・出典あり**（「実装後の訂正」参照）。分類は「出典なし」ではなく**「先送り」**が正しい |
...
- ~~**凛夏（りんか）**: りんごではなくニホンナシの品種の疑いが濃厚。品種マスタから削除すべきか要相談~~ → **解決済み(2026-07-20)。品種登録DB でリンゴ属と確認できたため、行は削除しない。** この項目を根拠に凛夏の行を消してはならない（「実装後の訂正」参照）
- **インド（カタカナ）**: 既存の基本品種「印度」(indo)と同一品種の可能性が高い重複行。統合・削除すべきか要相談
- ~~**出典が個人ブログ/Wikipedia の登録品種**（`おいらせ`・`秋陽`・`金星`・`ジャンボ王林`）~~ → **2026-07-20 の監査で書籍出典に差し替え済み**（「実装後の訂正」参照）。この項目は対応済みで、再着手は不要
...
- **コミット承認前の独立監査は audit-plan-step スキル**を使う。特に「テーブルの pollen/seed が varieties.md に正しく転記されたか」「baseline 差分が登録内容と一致するか」を機械照合する
  - **この照合で「実装後の訂正」の7品種に差分が出るのは正常。** 訂正済みの意図的な差分であり、varieties.md 側を表に合わせて直してはならない。表との差分が訂正一覧に載っていない場合のみ、想定外として報告する
